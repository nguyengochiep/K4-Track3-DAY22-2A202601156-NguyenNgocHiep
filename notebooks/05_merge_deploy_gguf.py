...
# NB3 continues training the SFT adapter with DPO, so adapters/dpo/ IS the aligned
# end state — that is what we merge. Merging adapters/sft-mini/ here (as this cell
# used to) would silently ship a GGUF with none of the DPO alignment in it.
model = PeftModel.from_pretrained(model, str(DPO_PATH))
print(f"Loaded DPO adapter from {DPO_PATH}")

# %% [markdown]
# > **Note:** Check the "Adapters present" line printed by NB3 § 1. If it lists a
# > single adapter, `adapters/dpo/` already contains SFT+DPO and merging it alone is
# > correct. If NB3 reported *two* adapters, load `adapters/sft-mini/` first and then
# > `model.load_adapter(DPO_PATH, ...)` before merging.
...
# %%
# Merges the DPO adapter into the base weights.
